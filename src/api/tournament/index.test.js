...
test('POST /tournaments 201 (admin)', async () => {
  ...
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ access_token: adminSession, name: 'test', type: 'test', game: id, dateStart: date, dateEnd: date, players: id, event: id, series: id, bracket: 'test', bracketImage: 'test', meta: 'test' })
...
test('PUT /tournaments/:id 404 (admin)', async () => {
  const { status } = await request(app())
    ...
    .send({ access_token: adminSession, name: 'test', type: 'test', game: 'test', dateStart: 'test', dateEnd: 'test', players: 'test', event: 'test', series: 'test', bracket: 'test', bracketImage: 'test', meta: 'test' })
  expect(status).toBe(404)
})
