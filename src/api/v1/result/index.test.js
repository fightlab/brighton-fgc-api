...
test('POST /results 201 (admin)', async () => {
  ...
  const { status, body } = await request(app())
    ...
    .send({ _playerId: id.toString(), _tournamentId: id.toString(), rank: 1, meta: 'test', eloBefore: 1000, eloAfter: 1001 })
  ...
  expect(body.eloBefore).toEqual(1000)
  expect(body.eloAfter).toEqual(1001)
...
test('PUT /results/:id 404 (admin)', async () => {
  const { status } = await request(app())
    ...
    .send({ _playerId: 'test', _tournamentId: 'test', rank: 'test', meta: 'test', eloBefore: 1000, eloAfter: 1001 })
  expect(status).toBe(404)
})
