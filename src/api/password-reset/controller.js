...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your brighton-fgc-api account.<br>
        ...
        &mdash; brighton-fgc-api Team
      `
      return sendMail({ toEmail: email, subject: 'brighton-fgc-api - Password Reset', content })
    })
